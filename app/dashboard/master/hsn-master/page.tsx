...
// NOTE: Put this file at: src/app/dashboard/master/hsn-master/page.tsx
// It reads data from: /api/master/hsn  (see route.ts)
//
...
// Formats a min/max pair as a single range string: "6%" or "6% – 12%"
...
type FieldDef = {
    key: string; // for "range" fields this is a synthetic id, not a raw DB key
    label: string;
    type?: FieldType;
    rangeKeys?: [string, string]; // [minKey, maxKey] for type "range"
};
...
const FIELD_GROUPS: { label: string; fields: FieldDef[] }[] = [
    ...
    {
        label: "Scheme Rate (RATE table — not GST)",
        fields: [
...
export default function HsnMasterPage() {
    ...
    const [coverage, setCoverage] = useState(""); // "", "with_products", "no_products"
    ...
    }, [rows, search, coverage, minProducts, maxProducts, minGst, maxGst]);
        ...
        const cols: ColumnDef<HsnRow, any>[] = ALL_FIELDS.map((f) => {
            ...
            return columnHelper.accessor(f.key, {
                ...
                cell: (info) => {
                    ...
                    if (f.type === "count") {
                        return (
                            <span className="inline-flex items-center justify-center min-w-[2.5rem] px-2 py-0.5 rounded-full text-xs font-semibold bg-indigo-500/15 text-indigo-700 ring-1 ring-indigo-500/30">
                                {Number(val || 0)}
                            </span>
                        );
                    }
                    if (f.type === "products") {
                        return (
                            <span
                                className="block max-w-[280px] truncate text-xs text-gray-500"
                                title={val || ""}
                            >
                                {val || "-"}
                            </span>
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
                <StatChip label="HSN Codes" value={totalHsn} tone="bg-blue-500" />
                <StatChip label="With Products" value={withProducts} tone="bg-emerald-500" />
                <StatChip label="Unused HSN" value={withoutProducts} tone="bg-rose-500" />
                <StatChip label="Total Products Mapped" value={totalProducts} tone="bg-indigo-500" />
            ...
            <div className="relative rounded-2xl overflow-hidden bg-white/60 backdrop-blur-xl border border-white/40 shadow-[0_4px_20px_rgba(0,0,0,0.06)]">
                ...
                <div className="relative flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-4 py-3 bg-gradient-to-r from-blue-600/85 to-indigo-600/85 backdrop-blur-md">
                    <div className="flex items-center gap-2">
                        ...
                        <h5 className="text-sm font-semibold text-white tracking-wide m-0">HSN Master — Full View</h5>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                        <div className="relative w-full sm:w-64">
                            ...
                            <input
                                ...
                                placeholder="Search HSN code, description, product..."
